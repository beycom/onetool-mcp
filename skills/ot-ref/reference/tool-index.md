...
## file, f
```python
file.copy(source: str, dest: str, follow_symlinks: bool=True)  # Copy a file or directory.
file.delete(path: str, backup: bool=True, recursive: bool=False, dry_run: bool=False)  # Delete a file or directory.
file.edit(path: str, old_text: str, new_text: str, occurrence: int=1, encoding: str='utf-8', dry_run: bool=False)  # Edit a file by replacing text.
...
## ground, g
```python
ground.dev(query: str, language: str='', framework: str='', timeout: float | None=None, max_sources: int | None=None, output_format: OutputFormat='full')  # Search for developer resources and documentation.
ground.docs(query: str, technology: str='', timeout: float | None=None, max_sources: int | None=None, output_format: OutputFormat='full')  # Search for official documentation.
ground.reddit(query: str, subreddit: str='', timeout: float | None=None, max_sources: int | None=None, output_format: OutputFormat='full')  # Search Reddit discussions.
...
mem.read(topic: str, id: str | None=None, meta: bool=False, mode: str | None=None)  # Read a memory by exact topic match or ID.
...
ot_context.grep(handle: str, pattern: str, context: int=0, store: HandleStore | None=None, config: Config | None=None)  # Regex line search with optional context lines and long-line truncation.
...
ot_context.purge(delete_all: bool=False, minutes: int=15, source: str='', status: str='', store: HandleStore | None=None)  # Delete handles matching the given filters.
...
ot_servers.restart(name: str)  # Reconnect a proxy server.
...
tavily.extract_batch(url_sets: list[tuple[list[str], str] | list[str]], format: str='markdown', extract_depth: str='basic')  # Extract content from multiple URL sets concurrently.
