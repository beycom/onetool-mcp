...
- [x] 1.1 In `src/ottools/ot_secrets.py`, add `_SECURE_BACKENDS` (set of fully-qualified class
...
- [x] 1.2 Call `_assert_secure_keyring_backend()` at the top of `init()` (`ot_secrets.py:68-103`,
...
- [x] 1.3 Call `_assert_secure_keyring_backend()` in the transparent-decrypt path in
  `src/ot/config/secrets.py` (`load_secrets()`, before the `keyring.get_password("onetool",
  "age_identity")` call at line 127).
- [x] 1.4 Unit test (`tests/ottools/unit/tools/test_secrets.py`, `@pytest.mark.unit
...
- [x] 1.5 Unit test: mock a secure backend (e.g. a `MagicMock` whose `type(...).__module__` /
  ...
  by existing tests).
- [x] 1.6 Unit test (`tests/unit/core/test_secrets.py`): mock an insecure backend for
  `load_secrets()`'s decrypt path and assert it raises before reading the identity.

## 2. Atomic write + 0600 helper (foundation)

- [x] 2.1 In `src/ottools/ot_secrets.py`, add `_atomic_write_yaml(path: Path, data: dict) -> None`
...
- [x] 2.2 Unit test: simulate a write failure (e.g. patch `yaml.dump` to raise mid-write) and
  assert the original `secrets.yaml` content is unchanged and no `.tmp_*` file is left behind.
- [x] 2.3 Unit test: assert the file mode of a freshly written `secrets.yaml` is `0o600` even when
...
- [x] 3.1 Wire task 1.2's `init()` call per design.md Decision 10 (already covered by 1.2 — this
...
- [x] 4.1 Change `encrypt()` signature default from `backup: bool = True` to `backup: bool = False`
  (`ot_secrets.py:106`).
- [x] 4.2 Replace the unconditional `shutil.copy2(path, backup_path)` (`ot_secrets.py:148-151`)
  with: only run when `backup=True`, and `os.chmod(backup_path, 0o600)` immediately after the
  copy.
- [x] 4.3 Replace the `with path.open("w") as f: yaml.dump(...)` write (`ot_secrets.py:171-172`)
  with a call to `_atomic_write_yaml()` from task 2.1.
- [x] 4.4 Add the `_assert_secure_keyring_backend()` call from task 1.2 before line 122's
  `keyring.get_password`.
- [x] 4.5 Unit test: `encrypt(file=...)` with no `backup=` argument — assert no `.bak` file is
  created and `result["backup"] is None`.
- [x] 4.6 Unit test: `encrypt(file=..., backup=True)` — assert `.bak` is created at mode `0o600`.
- [x] 4.7 Unit test: after `encrypt()`, assert `secrets.yaml`'s mode is `0o600`.
- [x] 4.8 Update `tests/ottools/unit/tools/test_secrets.py`'s existing `encrypt()` tests that
...
- [x] 5.1 Change `rotate()` signature default from `backup: bool = True` to `backup: bool = False`
  (`ot_secrets.py:253`).
- [x] 5.2 Replace the unconditional `shutil.copy2` backup (`ot_secrets.py:296-299`) with the same
  `backup=True`-gated + `chmod(0o600)` pattern as task 4.2.
- [x] 5.3 After re-encrypting each value with the new recipient (`ot_secrets.py:312-325`), add a
...
- [x] 5.4 Replace the `with path.open("w") as f: yaml.dump(...)` write (`ot_secrets.py:327-328`)
  with `_atomic_write_yaml()` from task 2.1.
- [x] 5.5 Reorder so the three `keyring.set_password` calls (`ot_secrets.py:330-332`, updating to
  ...
  Decision 7.
- [x] 5.6 Use `base64.b64decode(encoded, validate=True)` at `ot_secrets.py:318` instead of the
  current bare `base64.b64decode(encoded)`.
- [x] 5.7 Add the `_assert_secure_keyring_backend()` call from task 1.2 before the
  `keyring.get_password` calls at `ot_secrets.py:269-270`.
- [x] 5.8 Unit test: `rotate(file=...)` with no `backup=` argument — assert no `.bak` file.
- [x] 5.9 Unit test: `rotate(file=..., backup=True)` — assert `.bak` created at `0o600`.
- [x] 5.10 Unit test (crash simulation): patch the keychain update step (the third
...
- [x] 5.11 Unit test: patch the round-trip verification to detect a mismatch (e.g. force
...
- [x] 5.12 Update `tests/ottools/unit/tools/test_secrets.py`'s existing `rotate()` tests for the
  `backup=True` default flip, same as task 4.8.

## 6. `ot_secrets.py`: `file=` default resolution

- [x] 6.1 Add `_resolve_secrets_file(file: str | None) -> Path` per design.md Decision 6: explicit
  `file` wins (expanduser); else `ot.config.loader.get_loaded_secrets_path()`; else
  `ot.paths.get_config_dir() / "secrets.yaml"`.
- [x] 6.2 Change `encrypt()` (`ot_secrets.py:106`), `rotate()` (`:253`), and `audit()` (`:346`)
...
- [x] 6.3 Unit test: call `encrypt()` with no `file=` argument, with `ot.config.loader
  .get_loaded_secrets_path()` mocked to return a path — assert that path is used.
- [x] 6.4 Unit test: call `encrypt()` with no `file=` argument and no loaded secrets path — assert
  it falls back to `<config dir>/secrets.yaml` (mock `get_config_dir()`).

## 7. `ot_secrets.py`: `set()`

- [x] 7.1 Implement `set(*, key: str, value: str, file: str | None = None) -> dict[str, Any]` per
...
- [x] 7.2 Add `"set"` to `__all__` in `ot_secrets.py:23`.
- [x] 7.3 Unit test: `set(key="X", value="secret123")` with identity present — assert the written
...
- [x] 7.4 Unit test: `set()` overwriting an existing key — assert other keys/order unchanged.
- [x] 7.5 Unit test: `set()` with no identity present — assert plain value stored and `warning`
  field present recommending `init()`/`encrypt()`.
- [x] 7.6 Unit test: `set()` with an insecure keyring backend mocked — assert rejection before any
  file write (reuse the pattern from task 1.4).
- [x] 7.7 Unit test: `set()` with no `file=` — assert default resolution per task 6.

## 8. `ot_secrets.py`: `get()`

- [x] 8.1 Implement `get(*, key: str, file: str | None = None, out_file: str | None = None) ->
...
- [x] 8.2 Add `"get"` to `__all__` in `ot_secrets.py:23`.
- [x] 8.3 Unit test: `get(key="X")` (no `out_file`) for an existing encrypted key — assert result
...
- [x] 8.4 Unit test: `get(key="X", out_file=<tmp path>)` for an encrypted key — assert the tmp file
...
- [x] 8.5 Unit test: `get(key="MISSING")` — assert `{"found": False, "encrypted": None}`
  (or the chosen equivalent shape) with no error raised.
- [x] 8.6 Unit test: static/AST-level guard — assert `ot_secrets.get`'s source has no parameter
...
- [x] 9.1 Use `base64.b64decode(encoded, validate=True)` at `config/secrets.py:137` instead of the
  current bare `base64.b64decode(encoded)`.
- [x] 9.2 Align the "no identity" guidance text: pick one canonical message and apply it at both
...
- [x] 9.3 Unit test (`tests/unit/core/test_secrets.py`): assert `load_secrets()` raises
  `SecretDecryptionError` with the new canonical message text when no identity is present.
- [x] 9.4 Unit test: assert malformed (non-canonical) base64 in an `age1enc:` value raises a clear
...
- [x] 10.1 Extend the error string in `api_headers()` (`http.py:62`,
...
- [x] 10.2 Extend the error string in `require_api_key()` (`http.py:148`) the same way.
- [x] 10.3 Extend the error string in `check_api_key()` (`http.py:163`) the same way.
- [x] 10.4 Unit/integration test covering all three functions: assert the error message contains
  the secret name AND at least one of `"secrets.yaml"`, `"ot_secrets.set"`, or `"onetool init"`.
- [x] 10.5 `rg -n '"secret not configured"' packages/onetool-pack/src/otpack/http.py` — confirm all
...
- [x] 11.1 Add a small sanitizer (e.g. `_sanitize_connect_error(msg: str) -> str`) that strips/
  ...
  `manager.py`.
- [x] 11.2 Apply the sanitizer at `manager.py:489` (`self._errors[name] = str(e)` in the
  startup/parallel-connect path) and `manager.py:733` (`self._errors[name] = str(e)` in
  `connect_additional`).
- [x] 11.3 Unit test (`tests/unit/core/test_proxy_manager.py`): simulate a connection exception
...
- [x] 12.1 Add `ask_password_sync(prompt: str) -> str | None` to `src/ot/_tui.py`, following the
...
- [x] 12.2 Unit test: mock `questionary.password(...).ask()` to return a value, assert
  `ask_password_sync` returns it; mock it to raise `KeyboardInterrupt`, assert `None` is returned.

## 13. CLI: `onetool init` secrets step

- [x] 13.1 Add `"secrets.yaml"` to the `_exts` list in `src/onetool/cli.py:440-446`, with a
  description (e.g. `("secrets.yaml", "API keys / credentials (optionally encrypted)")`).
- [x] 13.2 In `init_callback()` (`cli.py:382-480`), when `"secrets.yaml"` is among `selected_ext`,
...
- [x] 13.3 After materialising `secrets.yaml`, if it was selected, prompt "Set up encrypted
  secrets?" (`questionary.confirm(..., default=False)`, matching the existing overwrite-confirm
  pattern at `cli.py:432-434`).
- [x] 13.4 If confirmed, loop: prompt for `key` via `ask_text_sync` (empty key stops the loop),
...
- [x] 13.5 After the loop (if at least one pair was entered and not cancelled), call
...
- [x] 13.6 Handle Ctrl+C during key/value entry per the "Cancel during encrypted-secrets key/value
...
- [x] 13.7 Unit/integration test (`tests/unit/core/test_init_redesign.py`): select `secrets.yaml`
...
- [x] 13.8 Unit/integration test: select `secrets.yaml`, confirm "Set up encrypted secrets?", enter
...
- [x] 13.9 Integration test (no mocks, using a real ephemeral keychain fixture if one exists in the
...
- [x] 14.1 Update the "Requires" section of `docs/reference/tools/ot_secrets.md:33-35` to list
  `pyrage` and `keyring` as Python package dependencies alongside the existing OS keychain support
  bullet.
- [x] 14.2 Update the "Functions" table in `docs/reference/tools/ot_secrets.md` (around lines
...
- [x] 14.3 Update the "Key Parameters" table's `backup` row (around line 30) to reflect the new
  `False` default.

## 15. Verification

- [x] 15.1 `rg -n "backup: bool = True" src/ottools/ot_secrets.py` — MUST return empty (both
  `encrypt()` and `rotate()` now default `backup=False`).
- [x] 15.2 `rg -n '"set", "status"' src/ottools/ot_secrets.py` or equivalent manual check —
  confirm `__all__` at `ot_secrets.py:23` includes `"get"` and `"set"` alongside the existing
  five names.
- [x] 15.3 `rg -n "shutil.copy2" src/ottools/ot_secrets.py` — every remaining call site MUST be
  inside a `if backup:` conditional followed by a `chmod(..., 0o600)` (manual inspection of each
  match, not just an empty-output check).
- [x] 15.4 `rg -n "path.open\(.w.\)" src/ottools/ot_secrets.py` — MUST return empty for the
  `encrypt`/`rotate`/`set` write paths (all writes now go through `_atomic_write_yaml`).
- [x] 15.5 `rg -n "b64decode\(encoded\)$" src/ottools/ot_secrets.py src/ot/config/secrets.py` —
  MUST return empty (both call sites now pass `validate=True`).
- [x] 15.6 `rg -n 'secret not configured"' packages/onetool-pack/src/otpack/http.py` — inspect all
  three matches (lines 62, 148, 163) and confirm each includes setup guidance beyond the bare
  secret name.
- [x] 15.7 No standalone `rg` check applies here — verify via test 13.7 (asserting
...
- [x] 15.8 `uv run pytest tests/ottools/unit/tools/test_secrets.py -m "unit and tools" -v` — all
  pass, including new `set`/`get`/backend-validation/atomic-write/rotate-crash-simulation tests.
- [x] 15.9 `uv run pytest tests/unit/core/test_secrets.py -m unit -v` — all pass, including the new
  base64-strict and guidance-message tests.
- [x] 15.10 `uv run pytest tests/unit/core/test_init_redesign.py -m unit -v` — all pass, including
  the new secrets-step tests.
- [x] 15.11 `uv run pytest tests/unit/core/test_proxy_manager.py -m unit -v` — all pass, including
  the new connect-error sanitization test.
- [x] 15.12 `just check` — lint (ruff) + typecheck (mypy strict) + full test suite all pass.
