...
- [ ] 1.1 Insert `"pydoll-python>=2.22.1",       # Chrome CDP driver for whiteboard pack` into the
...
- [ ] 1.2 Delete the entire `whiteboard = [ "pydoll-python>=2.22.1", ]` block (`pyproject.toml:79-81`),
      including its blank-line separation from the surrounding blocks.
- [ ] 1.3 Confirm `all = ["onetool-mcp[util,dev]"]` (`pyproject.toml:82`) is unchanged — do NOT add
...
- [ ] 1.4 Run `rg -n "whiteboard" pyproject.toml` and confirm it returns NO matches (no leftover
...
- [ ] 2.1 Run `uv lock` (NOT `uv lock --upgrade`) to regenerate `uv.lock` from the edited
      `pyproject.toml`.
- [ ] 2.2 Inspect the `uv.lock` diff (`git diff uv.lock`) and confirm it touches ONLY:
...
- [ ] 2.3 If `uv lock` cannot run (e.g. no package-index network access in the implementation
...
- [ ] 3.1 Change the `ImportError` message at `src/otdev/tools/excalidraw.py:107-108` from
...
- [ ] 3.2 Do NOT add `ChromiumOptions` to `_open_browser()`'s existing
...
- [ ] 3.3 Add a module-level helper function `_chrome_options() -> Any`, defined just above
...
- [ ] 3.4 Change `b = Chrome()` (`src/otdev/tools/excalidraw.py:119`, inside the `for attempt in
...
- [ ] 3.5 Re-read the edited `_open_browser()` function and confirm there is no remaining code path
...
- [ ] 4.1 In `tests/unit/tools/test_excalidraw.py`, add a unit test (marked `@pytest.mark.unit` and
      ...
      checked).
- [ ] 4.2 In `tests/unit/tools/test_excalidraw.py`, add a second unit test that exercises
...
- [ ] 4.3 Run `uv run pytest -m "unit and tools" tests/unit/tools/test_excalidraw.py -k chrome_option`
      (adjust the `-k` filter to match your actual test names) and confirm both new tests pass.

## 5. Documentation — extras contract

- [ ] 5.1 `docs/reference/tools/whiteboard.md:58` — change
...
- [ ] 5.2 `README.md` — in the tools table, the `whiteboard` row (currently at `README.md:159`:
      `` | `whiteboard`  | `open`, `draw`, `screenshot`, `save`           | `[dev]`  | Live Excalidraw canvas         | ``)
      — change the extras column from `` `[dev]` `` to `` `[util]` ``.
- [ ] 5.3 `docs/learn/installation.md` — in the "Optional Tool Packs" table
...
- [ ] 5.4 `docs/reference/tools/index.md` — in the "Optional Extras" table
      ...
      from `` `[dev]` `` to `` `[util]` ``.
- [ ] 5.5 `docs/llms.txt:53` — change `` | `whiteboard` | `[dev]` | Live whiteboard using Excalidraw...``
      to use `` `[util]` `` instead of `` `[dev]` ``.
- [ ] 5.6 Run `rg -n "\[whiteboard\]" src/ docs/ README.md` and confirm it returns NO matches (the
      only prior occurrence, `docs/reference/tools/whiteboard.md:58`, was fixed in task 5.1).
- [ ] 5.7 Do NOT edit `CHANGELOG.md` or `docs/learn/whats-new-v2.md` — both are historical release
...
- [ ] 6.1 In `docs/reference/tools/chrome-util.md`, under the `## Requires` section (currently just
...
- [ ] 6.2 Make the equivalent addition to `docs/reference/tools/play-util.md`'s `## Requires` section
...
- [ ] 7.1 `rg -n "whiteboard" pyproject.toml` → NO matches (task 1.4).
- [ ] 7.2 `rg -n "\[whiteboard\]" src/ docs/ README.md` → NO matches (task 5.6).
- [ ] 7.3 `git diff uv.lock` → touches only `pydoll-python`'s extra marker + removed `whiteboard`
...
- [ ] 7.5 `uv run pytest -m "unit and tools" tests/unit/tools/test_excalidraw.py` → all tests pass,
      including the two new tests from group 4.
- [ ] 7.6 `uv run python scripts/check_docs_registry.py` → passes (repo doc-registry gate; this
...
- [ ] 7.7 `just check` → passes (lint + typecheck + test, `--all-extras`). Note per design.md: this
