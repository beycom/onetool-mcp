...
- [ ] 1.1 In `src/ot/executor/pack_proxy.py`, inside `_create_mcp_proxy_pack.__getattr__`
...
- [ ] 1.2 Confirm no other caller of `_create_mcp_proxy_pack` or `McpProxyPack.__getattr__`
...
- [ ] 1.3 Add a unit test in `tests/unit/core/test_pack_proxy.py`, in a new test class following
...
- [ ] 2.1 In `src/ot/meta/_help_formatting.py`, in `_format_server_help` (currently lines
...
- [ ] 2.2 Add a unit test — either in `tests/unit/core/test_servers_redesign.py` (which already
...
- [ ] 3.1 In `src/ot/executor/runner.py`, in `execute_command`'s `except Exception as e:` block
...
- [ ] 3.2 Add a unit test that exercises `execute_command` (or `runner.execute_python_code` if
...
- [ ] 3.3 Add a second assertion (in the same test or a sibling test) for the no-plausible-match
...
- [ ] 4.1 In `src/ot/meta/_discovery.py`, in `tool_info` (currently lines 126-215), collect an
...
- [ ] 4.2 Replace the exact-name-miss return (currently line 213: `return {}`) with `return
...
- [ ] 4.3 Add a unit test in `tests/unit/tools/test_info.py` (existing file, already has
...
- [ ] 4.4 Add a second test asserting the no-plausible-match case: `tool_info(name=
...
- [ ] 5.1 In `src/ot/ctx/result_backend.py`, replace `CtxResultStoreBackend
...
- [ ] 5.2 Update `tests/unit/core/test_force_context_dunder.py::test_deflect_summary_includes_
  ...
  no longer asserted).
- [ ] 5.3 Confirm `src/ot/executor/result_store.py` (lines 66-69, the `QueryResult.to_dict`
...
- [ ] 6.1 Run the four new/updated unit test targets directly and confirm they pass:
...
- [ ] 6.2 Run `rg -n "ctx\.(toc|read|slice|grep|query|ask)" src/ot/executor/result_store.py
  ...
  complete.
- [ ] 6.3 Run `rg -n "next_commands" tests/ src/` and confirm every remaining occurrence reflects
...
- [ ] 6.4 Run the full unit test marker set for this change's touched areas:
  ...
  check).
- [ ] 6.5 Run `just check` (lint + type + test) and confirm it passes clean. This is the repo-wide
  ...
  in 6.1-6.4 alone.
- [ ] 6.6 Manually re-read the final diff for `src/ot/executor/runner.py`'s exception handler and
