...
- [ ] 1.1 Run `uv pip list --outdated` at the repo root and save the output for later comparison.
...
- [ ] 1.2 Confirm `packages/onetool-pack` is a uv workspace member with no separate lock file:
...
- [ ] 2.1 Run `uv lock --upgrade` at the repo root. This refreshes all direct + transitive
      dependencies for both the root project and the `packages/onetool-pack` workspace member in
      one pass (design.md D1).
- [ ] 2.2 Run `uv sync --group dev --all-extras` to materialize the refreshed environment. If
...
- [ ] 3.1 `pyproject.toml:23` — change `"fastmcp>=3.1.1,<4"` to `"fastmcp>=3.4.1,<4"`. This is a
...
- [ ] 3.2 `pyproject.toml:26` — raise `"mcp>=1.27.0"` to the version `uv.lock` resolved in task 2.1
      (measured 2026-07-04: `1.28.1`; re-check `uv.lock`'s actual `mcp` entry at implementation
      time and use that number).
- [ ] 3.3 `pyproject.toml:27` — check whether `pydantic` appears in task 1.1's outdated list. If it
...
- [ ] 3.4 `pyproject.toml:37` — raise `"openai>=2.32.0"` to the version `uv.lock` resolved in task
      2.1 (measured 2026-07-04: `2.44.0`; re-check at implementation time).
- [ ] 3.5 `pyproject.toml:44` — raise `"uvicorn>=0.46.0"` to the version `uv.lock` resolved in task
      2.1 (measured 2026-07-04: `0.49.0`; re-check at implementation time).
- [ ] 3.6 Do NOT add a direct `starlette` dependency line to `pyproject.toml`. Confirm instead that
...
- [ ] 3.7 `packages/onetool-pack/pyproject.toml:8-11` — check `loguru>=0.7.3`, `httpx>=0.28.1`,
...
- [ ] 4.1 Identify and run the test coverage that exercises `trafilatura`, `beautifulsoup4[lxml]`,
...
- [ ] 4.2 If all three pass: change `pyproject.toml:69` from `"lxml>=5.3,<6"` to
      `"lxml>=5.3,<7"`.
- [ ] 4.3 If any of the three fails: leave `pyproject.toml:69` as `"lxml>=5.3,<6"`. Do not force the
...
- [ ] 5.1 Run `just check` (lint + typecheck + non-integration test). Must exit 0.
- [ ] 5.2 Run the direct-run sanity recipe from `tests/explore/test-cli.md` lines 121-129 (section
...
- [ ] 5.3 Run the whiteboard/pydoll smoke: `uv run --all-extras pytest -m pydoll -v` (covers
      `tests/otdev/integration/tools/test_excalidraw.py`). Must pass.
- [ ] 5.4 Run the playwright/patchright smoke: `uv run --all-extras pytest -m playwright -v`
      (covers `tests/otdev/integration/tools/test_play_util.py`). Must pass.
- [ ] 5.5 Run the pymupdf smoke: `uv run --all-extras pytest tests/integration/tools/test_convert.py
      tests/otutil/unit/tools/test_convert.py -v`. Must pass. (No dedicated `pymupdf` pytest
      marker exists — run by path.)
- [ ] 5.6 Run `uv pip list --outdated` again (post-refresh). Confirm no direct dependency remains
...
- [ ] `rg -n "fastmcp>=" pyproject.toml` → must show `fastmcp>=3.4.1,<4`
- [ ] `rg -n "lxml>=" pyproject.toml` → must show either `lxml>=5.3,<7` (widened, task 4.2) or
      `lxml>=5.3,<6` (held, task 4.3 — with the failing package/test recorded below)
- [ ] `uv pip list --outdated` → no direct dependency listed, OR each remaining one listed here with
      name + current vs latest + reason:
      - (fill in at implementation time, or delete this bullet if the list is empty)
- [ ] `just check` → passes (lint + typecheck + test)
- [ ] `uv run --all-extras pytest -m pydoll -v` → passes
- [ ] `uv run --all-extras pytest -m playwright -v` → passes
- [ ] `uv run --all-extras pytest tests/integration/tools/test_convert.py
      tests/otutil/unit/tools/test_convert.py -v` → passes
- [ ] Direct-run sanity recipe (`tests/explore/test-cli.md:121-129`) → all 7 commands succeed
      against a running root process
- [ ] If lxml cap was held at `<6` (task 4.3): failing package + test/error recorded here —
      (fill in at implementation time, or delete this bullet if the cap was widened)
