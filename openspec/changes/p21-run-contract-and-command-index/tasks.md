...
- [ ] 1.1 Replace `src/ot/config/global_templates/prompts.yaml` with the exact content in design §A
- [ ] 1.2 Update prompt-content unit tests to assert the new content (new examples list, single
...
- [ ] 2.1 Extract `signature_args`/`short_description` helpers to a core-visible module (e.g.
      `src/ot/meta/_signatures.py`); make `src/otdev/docsgen/tool_index.py` import them from
      there (`ot` core MUST NOT import `otdev.*`)
- [ ] 2.2 Add `"signatures"` to `InfoLevel` (`src/ot/meta/_constants.py`) and
...
- [ ] 2.3 Unit tests: signatures level returns the one-liner format; single-pack pattern returns
...
- [ ] 3.1 Replace `skills/ot-ref/SKILL.md` with the exact content in design §B
- [ ] 3.2 Write `skills/ot-ref/agents/openai.yaml` with the exact content in design §C
      (implicit invocation MUST remain allowed — never `false`)
- [ ] 3.3 Write `skills/ot-ref/reference/recovery.md` with the exact content in design §D
- [ ] 3.4 Delete any leftover old ot-ref body content (the "ot-ref is optional" opener MUST NOT
      survive anywhere): `rg -n "ot-ref is optional" .` returns nothing

## 4. Generators (design §E–§F)

- [ ] 4.1 Extend `src/otdev/docsgen/tool_index.py` `main()` to also write
...
- [ ] 4.2 Add a staleness check (in `scripts/check_docs_registry.py` or the generated-blocks
      check) failing when the skill copy differs from a fresh generation; regenerate and commit
      the current index
- [ ] 4.3 New `src/otdev/docsgen/skill_pack_map.py` (logic ported from
...
- [ ] 4.4 Delete `_build_pack_summary()` and the `{pack_summary}` branch in `_get_instructions()`
      from `src/ot/server.py`; update/remove their tests
- [ ] 4.5 Unit tests: docsgen emits both index copies identical; pack-map generator includes
      aliases (`whiteboard` shows `wb`); marker block round-trips idempotently

## 5. Verification

- [ ] 5.1 `rg -n "pack_summary" src/ot/` returns nothing
- [ ] 5.2 `rg -n "ot\.skills" src/ docs/ README.md tests/` returns nothing (p11 owns the bulk;
      confirm this change reintroduced none)
- [ ] 5.3 `rg -n "brave.search" src/ot/config/global_templates/prompts.yaml` returns nothing
      (no key-gated example)
- [ ] 5.4 The colon rule appears exactly once in prompts.yaml (`rg -c "colon|snippets only"` —
      inspect manually; one statement with the right/wrong pair)
- [ ] 5.5 `diff docs/reference/tools/tool-index.md skills/ot-ref/reference/tool-index.md`
      is empty after a fresh generation run
- [ ] 5.6 `uv run python scripts/check_docs_registry.py` passes
- [ ] 5.7 `just check` passes
