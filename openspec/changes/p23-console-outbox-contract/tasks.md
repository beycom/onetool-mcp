...
- [ ] 1.1 Confirm `feature/display` still has the source content at
...
- [ ] 1.2 Run `openspec archive` (or the project's `opsx:sync`/`opsx:apply` flow) to apply this change's
...
- [ ] 2.1 After sync/archive, `openspec/specs/console-outbox/spec.md` will have a Purpose section reading
...
- [ ] 2.2 Replace that placeholder Purpose section with:
      ...
      **Status: protocol v1 — server implementation ships with display (3.1).** This capability defines
      ...
      release 3.1.
      ```
- [ ] 2.3 Confirm the resulting `openspec/specs/console-outbox/spec.md` Requirements section (everything
...
- [ ] 3.1 Copy `feature/display:docs/reference/console-outbox-protocol.md` to
...
- [ ] 3.2 Add the nav entry to `mkdocs.yml` under the `Reference:` block, between the `CLIs:` sub-block
...
- [ ] 4.1 Create `tests/fixtures/console-protocol/schemas/event-envelope.schema.json` — copy verbatim from
      `feature/display:tests/fixtures/console-protocol/schemas/event-envelope.schema.json`.
- [ ] 4.2 Create `tests/fixtures/console-protocol/schemas/instance-snapshot.schema.json` — copy verbatim
      from `feature/display:tests/fixtures/console-protocol/schemas/instance-snapshot.schema.json`.
- [ ] 4.3 Create `tests/fixtures/console-protocol/schemas/display-message.schema.json` — copy verbatim from
      `feature/display:tests/fixtures/console-protocol/schemas/display-message.schema.json`.
- [ ] 4.4 Create `tests/fixtures/console-protocol/schemas/outbox-batch.schema.json` — copy verbatim from
...
- [ ] 4.5 Create `tests/fixtures/console-protocol/schemas/outbox-ack.schema.json` — copy verbatim from
      `feature/display:tests/fixtures/console-protocol/schemas/outbox-ack.schema.json`.
- [ ] 4.6 Verify all five schema files are byte-identical to their branch sources:
...
- [ ] 5.1 Create `tests/fixtures/console-protocol/fixtures/display-message-event.json` — copy verbatim
      from `feature/display:tests/fixtures/console-protocol/fixtures/display-message-event.json`.
- [ ] 5.2 Create `tests/fixtures/console-protocol/fixtures/empty-outbox-batch.json` — copy verbatim from
      `feature/display:tests/fixtures/console-protocol/fixtures/empty-outbox-batch.json`.
- [ ] 5.3 Create `tests/fixtures/console-protocol/fixtures/file-diff-ref-display-message.json` — copy
      verbatim from `feature/display:tests/fixtures/console-protocol/fixtures/file-diff-ref-display-message.json`.
- [ ] 5.4 Create `tests/fixtures/console-protocol/fixtures/file-ref-display-message.json` — copy verbatim
      from `feature/display:tests/fixtures/console-protocol/fixtures/file-ref-display-message.json`.
- [ ] 5.5 Create `tests/fixtures/console-protocol/fixtures/inline-display-message.json` — copy verbatim
      from `feature/display:tests/fixtures/console-protocol/fixtures/inline-display-message.json`.
- [ ] 5.6 Create `tests/fixtures/console-protocol/fixtures/instance-snapshot.json` — copy verbatim from
      `feature/display:tests/fixtures/console-protocol/fixtures/instance-snapshot.json`.
- [ ] 5.7 Create `tests/fixtures/console-protocol/fixtures/outbox-ack.json` — copy verbatim from
      `feature/display:tests/fixtures/console-protocol/fixtures/outbox-ack.json`.
- [ ] 5.8 Create `tests/fixtures/console-protocol/fixtures/outbox-batch.json` — copy verbatim from
      `feature/display:tests/fixtures/console-protocol/fixtures/outbox-batch.json`.
- [ ] 5.9 Verify all eight fixture files are byte-identical to their branch sources:
...
- [ ] 6.1 Create `tests/unit/core/test_console_protocol_fixtures.py` containing **only** the
...
- [ ] 6.2 Keep the branch's `_schema()` helper that inlines `event-envelope.schema.json` into
...
- [ ] 6.3 Confirm `jsonschema` imports successfully without any `pyproject.toml`/`uv.lock` change:
      `uv run python -c "import jsonschema; print(jsonschema.__version__)"`.
- [ ] 6.4 Run the new test in isolation: `uv run pytest tests/unit/core/test_console_protocol_fixtures.py -v` — all cases in the (single) test function's fixture/schema pairs must pass.

## Verification

- [ ] V.1 `test -f openspec/specs/console-outbox/spec.md` succeeds and the file's Requirements section
      matches `feature/display:openspec/specs/console-outbox/spec.md` (task 2.3).
- [ ] V.2 `test -f docs/reference/console-outbox-protocol.md` succeeds and
      `git show feature/display:docs/reference/console-outbox-protocol.md | diff - docs/reference/console-outbox-protocol.md`
      produces no output.
- [ ] V.3 `grep -n "Console Outbox Protocol: reference/console-outbox-protocol.md" mkdocs.yml` finds the
      nav entry added in task 3.2.
- [ ] V.4 All five schema diffs and all eight fixture diffs from tasks 4.6 and 5.9 produce no output.
- [ ] V.5 `uv run pytest tests/unit/core/test_console_protocol_fixtures.py -v` passes.
- [ ] V.6 `rg -n "console_outbox" src/` returns **empty** (no runtime code was added). This is a hard
      acceptance gate from report R6 — if this command returns any match, the change is not done.
- [ ] V.7 `rg -n "ot\.display" src/ --glob '!**/__pycache__/**'` returns **empty** (no display-pack code
...
- [ ] V.8 Confirm no new HTTP route was registered: `rg -n "api/console/outbox" src/` returns empty (the
...
- [ ] V.10 `just check` (lint + typecheck + test) passes.
