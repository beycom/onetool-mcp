# Capability: backend-onetool-dev

**Purpose:** Developer tools backend server for database operations, code search, web fetching, package information, documentation search, diagram rendering, and browser automation.

**Scope:** Standalone MCP server that can operate independently or be proxied by onetool-mcp frontend.
...
- **onetool-common** (capability: shared-library) - REQUIRED for config, logging, paths, batch, pathsec, factory utilities
- **FastMCP** (external) - REQUIRED for MCP server framework
- **rg CLI** (external) - REQUIRED for ripgrep pack

### Related Capabilities

- **backend-onetool-util** (add-onetool-util) - Parallel backend for utility tools
- **backend-onetool-xero** (add-onetool-common) - Example backend using same pattern
- **frontend-onetool-mcp** (refactor-onetool-mcp) - Future capability, will proxy to this backend

### Sequencing

1. **Prerequisite:** onetool-common must exist (add-onetool-common completed)
2. **This capability:** onetool-dev backend created
3. **Enables:** onetool-mcp refactor (Proposal 4) can proxy to onetool-dev
