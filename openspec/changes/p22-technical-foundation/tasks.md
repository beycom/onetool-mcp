...
- [ ] 1.1 In `dev/project/arch/security-model.md`, rewrite the "Layer 3: Namespace Restriction"
...
- [ ] 1.2 In the same section (or a new "Deferred hardening" note), record that builtins-narrowing /
      an alternative sandbox (e.g. Monty) is deferred to V4, contingent on the threat model changing
      — not implemented in this change.
- [ ] 1.3 While in the file, verify the Layer 2 example `security.yaml` snippet (currently lines
...
- [ ] 1.4 Do not modify `src/ot/executor/runner.py` or `src/ot/executor/validator.py` in this task
...
- [ ] 2.1 In `src/ottools/ot_llm.py`, locate `transform()`'s system message construction (currently
...
- [ ] 2.2 Add a unit test in `tests/ottools/unit/tools/test_llm.py` asserting the constructed
...
- [ ] 2.3 In `src/otutil/tools/_knowledge/retrieval.py`, locate `_synthesise()` (currently lines
...
- [ ] 2.4 Add a unit test for `_synthesise()` and `_llm_rerank()` (new or extend
...
- [ ] 2.5 Confirm (do not re-implement) that `ctx.ask()` (`src/ot/ctx/ask.py`) and `mem.ask()`
...
- [ ] 3.1 Create `src/ot/logging/redact.py` with a `SECRET_PATTERNS` list containing the ten
...
- [ ] 3.2 In `src/ot/logging/format.py`'s `sanitize_for_output()` (currently lines 142+), call
      ...
      `ot.logging.redact`.
- [ ] 3.3 In `src/ot/logging/entry.py`'s `LogEntry.__str__()` (currently lines 184-202), replace the
...
- [ ] 3.4 In `src/otutil/tools/_mem/config.py`, delete the local `_BUILTIN_REDACTION_PATTERNS`
      ...
      a direct import.
- [ ] 3.5 In `src/otutil/tools/_mem/content.py:16`, update the import of `_BUILTIN_REDACTION_PATTERNS`
...
- [ ] 3.6 Add a unit test in `tests/unit/core/test_log_format.py` (or `test_format.py`) asserting: a
      ...
      Decision 3.
- [ ] 3.7 Add a unit test asserting the same redaction applies to a field named `preparedCode` and a
...
- [ ] 4.1 Add `validate_batch_retry_controls(retries: int, retry_delay_ms: int) -> str | None` to
...
- [ ] 4.2 Delete `_validate_batch_retry_controls` from `brave.py`, `tavily.py`, `ground.py`. Update
      each call site (`brave.py:801`, `tavily.py:861`, `ground.py:608`) to call
      `otpack.validate_batch_retry_controls(...)` (import at module top).
- [ ] 4.3 Add `format_sources(results: list[dict[str, Any]], *, max_sources: int | None = None) -> str`
...
- [ ] 4.4 Delete `_format_sources` from `brave.py`, `tavily.py`, `ground.py`. Update all call sites
...
- [ ] 4.5 Add `create_json_http_client(base_url: str, *, timeout: float | Any = 30.0, headers:
...
- [ ] 4.6 Add `extract_structured_data(*, text: str, sources: list[dict[str, Any]], extract_schema:
...
- [ ] 4.7 Run `packages/onetool-pack`'s boundary check
...
- [ ] 4.8 Add unit tests for the four new/extended `otpack` functions
...
- [ ] 4.9 Run the existing search-pack test suites
...
- [ ] 5.1 **Precondition check (stop-and-report if it fails):** confirm `src/ottools/skills.py` and
      ...
      as a workaround.
- [ ] 5.2 Add `parse_frontmatter(content: str) -> tuple[dict[str, Any], str]` to
...
- [ ] 5.3 In `src/otutil/tools/_knowledge/chunker.py`, delete `_parse_frontmatter` (lines 42-62) and
...
- [ ] 5.4 In `src/ottools/ot_forge.py`, locate the ad-hoc frontmatter parsing in `_get_skill_description`
...
- [ ] 5.5 Add a unit test for `otpack.parse_frontmatter` in `packages/onetool-pack/tests/` covering:
...
- [ ] 6.1 Confirm `src/ot/display/` contains no `.py` source files — only `__pycache__/*.pyc` and
      `assets/__pycache__/*.pyc` (verified during design; re-verify at implementation time in case the
      tree changed).
- [ ] 6.2 Confirm no source file references `ot.display` or `ot/display` outside of
...
- [ ] 6.3 Delete `src/ot/display/` entirely (`rm -rf src/ot/display/`).
- [ ] 6.4 Do **not** touch `src/ot/server.py:208-237`'s `_build_pack_summary()` function or the
...
- [ ] 7.1 In `tests/conftest.py`'s `pytest_collection_modifyitems` (currently lines 141-167), change
...
- [ ] 7.2 Update the module docstring (currently lines 1-10) if needed so it accurately describes the
      ...
      `require()`-based ones.
- [ ] 7.3 Add a test fixture proving the new gate works: a deliberately unmarked test function (in a
...
- [ ] 7.4 Run `just test` (no `--allow-skips`) across the full suite once the fix lands, and confirm
...
- [ ] 8.1 `rg "only allowlisted builtins" dev/project/arch/security-model.md` — empty.
- [ ] 8.2 `rg "Excluded:.*__import__" dev/project/arch/security-model.md` — empty (confirms the false
      exclusion claim is gone).
- [ ] 8.3 Manually confirm `dev/project/arch/security-model.md`'s Layer 3 section states "not a
      sandbox" and the process/user/environment trust-boundary language from task 1.1.
- [ ] 8.4 `uv run pytest tests/ottools/unit/tools/test_llm.py -m unit` — passes, including the new
      untrusted-data boundary test (task 2.2).
- [ ] 8.5 `uv run pytest tests/otutil/unit/tools/test_knowledge.py -m unit` — passes, including the new
      `_synthesise`/`_llm_rerank` boundary tests (task 2.4).
- [ ] 8.6 `uv run pytest tests/unit/core/test_log_format.py tests/unit/core/test_format.py -m unit` —
      passes, including the new secret-redaction tests (tasks 3.6, 3.7) for both `LogSpan` and direct
      `LogEntry` paths.
- [ ] 8.7 `uv run pytest tests/otutil/unit/tools/test_mem.py tests/integration/tools/test_mem.py` —
      passes unmodified, confirming `mem.write()` redaction behavior is unchanged after task 3.4/3.5's
      relocation.
- [ ] 8.8 `rg "_validate_batch_retry_controls" src/otutil/tools/` — shows only the new
...
- [ ] 8.9 `rg "^def _format_sources|^def _extract_structured_data|^def _create_http_client" src/otutil/tools/brave.py src/otutil/tools/tavily.py src/otutil/tools/ground.py`
      — empty (confirms the local definitions were deleted, not just supplemented).
- [ ] 8.10 `uv run --directory packages/onetool-pack pytest -m unit` — passes, including the new
      `otpack.batch`/`.text`/`.http` helper tests (task 4.8) and boundary check (task 4.7).
- [ ] 8.11 `uv run pytest tests/otutil/unit/tools/test_brave.py tests/otutil/unit/tools/test_tavily.py tests/otutil/unit/tools/test_ground.py tests/integration/tools/test_brave.py tests/integration/tools/test_ground.py tests/otutil/integration/tools/test_tavily.py`
      — passes unmodified (task 4.9's regression net).
- [ ] 8.12 `rg "def _parse_frontmatter" src/` — empty (confirms both `chunker.py` and `ot_forge.py`
      duplicates are gone; also confirms `p11`'s deletions of `skills.py`/`_skills_services.py`
      already removed their copies).
- [ ] 8.13 `uv run pytest tests/otutil/unit/tools/test_knowledge.py -m unit` and
...
- [ ] 8.14 `ls src/ot/display` — fails (directory does not exist).
- [ ] 8.15 `rg "ot\.display|ot/display" src tests docs openspec dev --glob '!**/node_modules/**'` —
...
- [ ] 8.16 `rg "pack_summary" src/ot/server.py` — still present (confirms task 6.4's boundary was
      respected; `_build_pack_summary` was NOT deleted by this change).
- [ ] 8.17 Run a deliberately unmarked test (temp file per task 7.3) and confirm it fails collection
      under `uv run pytest` (no `--allow-skips`) and is skipped under
      `uv run pytest --allow-skips`.
- [ ] 8.18 `just test` — zero tests silently skipped for missing markers across the full existing
      suite (task 7.4).
- [ ] 8.19 `just check` (lint + typecheck + test) — passes with zero errors, run last, after all
