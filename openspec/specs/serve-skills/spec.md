...
### Requirement: ot-use-worker skill distribution

OneTool SHALL distribute `ot-use-worker` as a standard Agent Skill at
`skills/ot-use-worker/SKILL.md`. It SHALL include Codex sidecar metadata
...
- **THEN** `skills/ot-use-worker/SKILL.md` SHALL exist with standard name and description frontmatter
- **AND** `skills/ot-use-worker/agents/openai.yaml` SHALL set `policy.allow_implicit_invocation` to `false`

#### Scenario: Skill is discoverable by a standard installer
- **WHEN** a standard Agent Skills installer lists repository skills
- **THEN** `ot-use-worker` SHALL be available for installation
