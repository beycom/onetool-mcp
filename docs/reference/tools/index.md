...
|-------|---------------|
| *(core, always included)* | `display`, `ot_caveman` (alias `cm`), `ctx`, `ot`, `ot_forge`, `ot_llm`, `ot_secrets`, `ot_servers`, `ot_timer` |
...
| [**Display**](display.md) | core | In-process message store for rich, user-visible artifacts inspected through the shared Admin App. | 7 | MIT | `clear`, `focus`, `list`, `read`, `seed_mock_messages`, `show`, `status` |
