...
- If `tools.diagram` is omitted, OneTool uses built-in backend/policy/output defaults.
- Template map defaults to empty (`templates: {}`) unless provided via config/include.
- Source-file provider inference supports compound extensions generated by this pack, including `.vg.json` (Vega) and `.vl.json` (Vega-Lite).
...
# Check async/batch status with progress counters
diagram.get_render_status(task_id="batch-abc123")
```

## Templates

OneTool includes built-in diagram templates for common patterns. Editable overrides are installed to `templates/diagram/` inside your config directory during `onetool init`.
...
Templates can be customized by editing files in the `templates/diagram/` directory inside your config directory.
