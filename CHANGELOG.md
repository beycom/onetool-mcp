...
### Context Store (`ctx`) Improvements
...
- **README updated** — added all v2 tool packs and install extras; fixed broken links.
...
- **Install extras** — `[util]` (file, excel, convert, brave, ground) and `[dev]` (db, ripgrep, web, diagram, package, context7); `[all]` installs everything.
- **`--secrets` flag** — on `onetool serve`.
- **`file` tool options** — `dry_run`, symlinks, `include_hidden`, recursive delete, encoding options.

### Changed

- **Global config location** — `~/.onetool/onetool.yaml` (was `~/.onetool/config/onetool.yaml`).
- **Zero-config start** — server starts with defaults if no config found; `onetool init` no longer required.
- **`[file]` extra** — folded into `[util]`.
...
- **Tool name aliasing** — automatic aliases for tool names that aren't Python-friendly.
- **Browser annotation packs** — `devtools_util` and `playwright_util`, with a Chrome DevTools guide.

### Fixed

- **Env var expansion** — env vars from secrets expand for stdio MCP servers; new `inherit_env` option.
...
- **Mem Tool** — 11–13× faster than accessing files directly, with an improved user experience.
- **Agent Hints** — agents can get help on using OneTool via `ot.agent_hints()`.
- **Timer Pack** — measure elapsed time across tool calls.
- **Dev Docs** — architecture, coding standards, and contributor guides documented for agents and contributors.

### Changed

- **`mem.grep`** — regex search across memories; updated prompts and guides.
...
- **Persistent memory tool** — added.
- **GitHub MCP** — fixed by implementing streamable HTTP transport.
- **Proxy server discovery** — added discovery and instructions; Chrome DevTools and GitHub MCP servers enabled by default.
- **`ot_llm`** — added `transform_file` and the `data` param.
- **Removed** — `code_search` and `timed` tools.
