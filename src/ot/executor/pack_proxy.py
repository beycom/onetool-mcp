...
def reset() -> None:
    ...
    Called by ot.reload() to release stale proxy/registry references, and by
    ProxyManager._evict_proxy_caches() after any server connect/disconnect/restart
    (D14): each cached namespace embeds per-server ``McpProxyPack`` objects whose
    ``_function_cache`` maps accessor names to tool closures resolved at first access,
    and on a *restart* (disconnect immediately followed by reconnect of the same
    server name) the outer namespace-cache key — ``frozenset(proxy_mgr.servers)`` — is
