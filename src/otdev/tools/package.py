    ...
    *,
    ...
    with LogSpan(span="package.audit", path=path, registry=registry) as span:
        ...
        with ThreadPoolExecutor(max_workers=min(len(deps), 20)) as executor:
            ...
            fetch_results = {name: f.result() for name, f in futures.items()}
...
def npm(*, packages: list[str]) -> list[dict[str, Any]] | str:
...
def pypi(*, packages: list[str]) -> list[dict[str, Any]] | str:
