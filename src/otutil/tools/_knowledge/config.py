...
class ScrapeSourceConfig(BaseModel):
    ...
    url_prefix: str = Field(
        default="", description="Restrict crawl to URLs with this path prefix"
    )
    depth: int | None = Field(
        default=None, description="BFS crawl depth (None = inherit from project)"
    )
    max_pages: int | None = Field(
        default=None, description="Max pages to crawl (None = inherit from project)"
    )
    check_robots_txt: bool | None = Field(
        default=None, description="Respect robots.txt (None = inherit from project)"
    )
    delay_min: float | None = Field(
        default=None, description="Min delay seconds (None = inherit from project)"
    )
    delay_max: float | None = Field(
        default=None, description="Max delay seconds (None = inherit from project)"
    )
    user_agent: str | None = Field(
        default=None, description="User-Agent string (None = inherit from project)"
    )
    wait_for: str | None = Field(
        default=None,
        description="CSS/JS selector to wait for before extracting (None = inherit from project)",
    )
    page_timeout: int | None = Field(
        default=None,
        description="Page load timeout in ms (None = inherit from project)",
    )
    process_iframes: bool | None = Field(
        default=None,
        description="Extract text from embedded iframes (None = inherit from project)",
    )
    content_filter_threshold: float | None = Field(
        default=None,
        description="PruningContentFilter threshold 0-1 (None = inherit from project)",
    )
    min_word_threshold: int | None = Field(
        default=None,
        description="Minimum words per block for PruningContentFilter (None = inherit from project)",
    )
    crawl_strategy: Literal["bfs", "dfs", "best_first", "seed_urls"] | None = Field(
        default=None,
        description="Crawl strategy override (None = inherit from project)",
    )
    seed_urls: list[str] = Field(
        default_factory=list,
        description="Explicit URL list — only used when crawl_strategy=seed_urls",
    )
    score: dict[str, Any] = Field(
        default_factory=dict,
        description="Scorer config for best_first strategy (e.g. {keyword_relevance: ['term:1.0']})",
    )
    css_selector: str = Field(
        default="",
        description="CSS selector to restrict content extraction to a specific element (e.g. '#mc-main-content')",
    )
    js_code: str = Field(
        default="",
        description="JavaScript to run on each page before extraction (crawl4ai js_code)",
    )
    include_images: bool | None = Field(
        default=None,
        description="Append image URLs to page content (None = inherit from project)",
    )
    flat_files: bool | None = Field(
        default=None,
        description="Write files with '::' separator instead of subdirectories (None = inherit from project)",
    )
    category: str | None = Field(
        default=None,
        description="Default chunk category for this source (None = inherit from project). Must be one of: reference, rule, note, mistake",
    )
    tags: list[str] | None = Field(
        default=None,
        description="Default tags to attach to all chunks from this source (None = inherit from project)",
    )
...
class ScrapeProjectConfig(BaseModel):
    ...
    output_base_dir: str = Field(
        description="Absolute base path; source output dirs are derived as output_base_dir/source_name"
    )
    user_agent: str = Field(
        default="", description="Custom User-Agent string (empty = crawl4ai default)"
    )
    delay_min: float = Field(
        default=0.5, description="Minimum delay between requests (seconds)"
    )
    delay_max: float = Field(
        default=2.0, description="Maximum delay between requests (seconds)"
    )
    ...
    wait_for: str = Field(
        default="",
        description="CSS/JS selector to wait for before extracting (empty = no wait)",
    )
    page_timeout: int = Field(default=30000, description="Page load timeout in ms")
    crawl_strategy: Literal["bfs", "dfs", "best_first", "seed_urls"] = Field(
        default="bfs",
        description="Default crawl strategy for all sources in this project",
    )
    score: dict[str, Any] = Field(
        default_factory=dict,
        description="Default scorer config for best_first strategy",
    )
    cache: bool = Field(default=False, description="Enable crawl4ai disk cache")
    process_iframes: bool = Field(
        default=False, description="Extract text from embedded iframes"
    )
    content_filter_threshold: float = Field(
        default=0.48, description="PruningContentFilter threshold 0-1"
    )
    min_word_threshold: int = Field(
        default=50, description="Minimum words per block for PruningContentFilter"
    )
    include_images: bool = Field(
        default=False, description="Append image URLs to page content"
    )
    flat_files: bool = Field(
        default=False,
        description="Write files with '::' separator instead of subdirectories",
    )
    category: str | None = Field(
        default=None,
        description="Default chunk category for all sources in this project. Must be one of: reference, rule, note, mistake",
    )
    tags: list[str] = Field(
        default_factory=list,
        description="Default tags to attach to all chunks in this project",
    )
    sources: dict[str, ScrapeSourceConfig] = Field(
        description="Named sources in this project"
    )
...
def _resolve_category(
    source_cat: str | None, project_cat: str | None, source_name: str
) -> str | None:
...
def resolve_source(
    ...
    crawl_strategy = (
        ...
        if source.crawl_strategy is not None
        else project.crawl_strategy
    )
    ...
    return ResolvedSourceConfig(
        ...
        if source.max_pages is not None
        else project.max_pages,
        check_robots_txt=source.check_robots_txt
        if source.check_robots_txt is not None
        else project.check_robots_txt,
        delay_min=source.delay_min
        if source.delay_min is not None
        else project.delay_min,
        delay_max=source.delay_max
        if source.delay_max is not None
        else project.delay_max,
        user_agent=source.user_agent
        if source.user_agent is not None
        else project.user_agent,
        wait_for=source.wait_for if source.wait_for is not None else project.wait_for,
        page_timeout=source.page_timeout
        if source.page_timeout is not None
        else project.page_timeout,
        cache=project.cache,
        process_iframes=source.process_iframes
        if source.process_iframes is not None
        else project.process_iframes,
        content_filter_threshold=source.content_filter_threshold
        if source.content_filter_threshold is not None
        else project.content_filter_threshold,
        min_word_threshold=source.min_word_threshold
        if source.min_word_threshold is not None
        else project.min_word_threshold,
        ...
        if source.include_images is not None
        else project.include_images,
        flat_files=source.flat_files
        if source.flat_files is not None
        else project.flat_files,
...
class DBConfig(BaseModel):
    ...
    embeddings_enabled: bool = Field(
        default=True, description="Enable vector embeddings for this DB"
    )
...
class KBProjectConfig(BaseModel):
    ...
    scrape: ScrapeProjectConfig | None = Field(
        default=None, description="Scrape project configuration (optional)"
    )
    index: IndexProjectConfig = Field(
        default_factory=IndexProjectConfig, description="Index configuration"
    )
