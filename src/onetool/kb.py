...
_DB_ARG = typer.Argument(
    help="Database / project name (as configured under kb: in onetool.yaml)."
)
...
def cmd_index(
    project: Annotated[
        str, typer.Argument(help="Project name from onetool.yaml kb config.")
    ],
    path: Annotated[
        str | None,
        typer.Option(
            "--path", help="Directory to index (overrides project's output_base_dir)."
        ),
    ] = None,
    overwrite: Annotated[
        str, typer.Option("--overwrite", help="'skip' (default) or 'update'.")
    ] = "skip",
    enrich: Annotated[
        bool,
        typer.Option(
            "--enrich", help="Generate LLM summaries for chunks indexed this run."
        ),
    ] = False,
) -> None:
    ...
    if overwrite not in ("skip", "update"):
        console.print(
            f"[red]Invalid --overwrite value '{overwrite}'. Must be 'skip' or 'update'.[/red]"
        )
        raise typer.Exit(1)
    ...
    if path is None:
        ...
        if kb_project is None:
            ...
            console.print(
                f"[red]Unknown project '{project}'. Available: {available}[/red]"
            )
            raise typer.Exit(1)
        if kb_project.scrape is None:
            console.print(
                f"[red]Project '{project}' has no scrape config — use --path to specify a directory.[/red]"
            )
            raise typer.Exit(1)
    ...
    with Progress(
        ...
        embed_task = progress.add_task(
            "[cyan]Embedding[/cyan]", total=None, visible=False, info=""
        )
        link_task = progress.add_task(
            "[cyan]Link graph[/cyan]", total=None, visible=False, info=""
        )
...
def cmd_enrich(
    ...
    limit: Annotated[
        int | None, typer.Option("--limit", help="Max chunks to enrich this run.")
    ] = None,
    force: Annotated[
        bool,
        typer.Option(
            "--force", help="Re-summarise all chunks, not just those missing summaries."
        ),
    ] = False,
) -> None:
    ...
    with Progress(
        ...
        try:
            result = enrich_db(
                db_name=db, limit=limit, force=force, on_progress=on_progress
            )
        except ValueError as exc:
            console.print(f"[red]{exc}[/red]")
            raise typer.Exit(1) from exc
...
def cmd_scrape(
    project: Annotated[
        str, typer.Argument(help="Project name from onetool.yaml scrape config.")
    ],
    only: Annotated[
        str | None, typer.Option("--only", help="Comma-separated source names to run.")
    ] = None,
    resume: Annotated[
        bool,
        typer.Option(
            "--resume", help="Resume each source from .state.json if present."
        ),
    ] = False,
    debug: Annotated[
        bool,
        typer.Option(
            "--debug",
            help="Write per-page debug artifacts (cleaned.html, raw.html, screenshot.png, meta.json) to ._debug/<slug>/ inside each source output dir.",
        ),
    ] = False,
    max_pages: Annotated[
        int | None,
        typer.Option(
            "--max-pages",
            help="Hard limit on pages written per source (overrides per-source and project config).",
        ),
    ] = None,
    flat_files: Annotated[
        bool | None,
        typer.Option(
            "--flat-files/--no-flat-files",
            help="Write flat '::'-separated files instead of subdirectories (overrides per-source config).",
        ),
    ] = None,
) -> None:
    ...
    if kb_project is None or kb_project.scrape is None:
        available = (
            ", ".join(sorted(k for k, v in cfg.kb.items() if v.scrape is not None))
            or "(none configured)"
        )
        console.print(
            f"[red]Unknown scrape project '{project}'. Available: {available}[/red]"
        )
        raise typer.Exit(1)
    ...
    for source_name, source in sources_to_run.items():
        ...
        if resolved.crawl_strategy != "seed_urls":
            if resolved.max_pages > 500:
                console.print(
                    f"[yellow]⚠  {source_name}: max_pages={resolved.max_pages} — large crawl[/yellow]"
                )
            if resolved.depth > 4:
                console.print(
                    f"[yellow]⚠  {source_name}: depth={resolved.depth} — deep crawl, may be slow[/yellow]"
                )
        if resolved.url_prefix == "":
            console.print(
                f"[yellow]⚠  {source_name}: no url_prefix — entire domain will be crawled[/yellow]"
            )
        if resolved.delay_min < 0.5:
            console.print(
                f"[yellow]⚠  {source_name}: delay_min={resolved.delay_min} — aggressive rate; may trigger blocks[/yellow]"
            )
        if (
            resolved.crawl_strategy in ("bfs", "dfs", "best_first")
            and resolved.seed_urls
        ):
            console.print(
                f"[yellow]⚠  {source_name}: seed_urls is set but crawl_strategy={resolved.crawl_strategy!r} — seed_urls will be ignored[/yellow]"
            )
        ...
        with Progress(
            ...
            _effective_max_pages = (
                max_pages if max_pages is not None else resolved.max_pages
            )
            _scrape_total = (
                len(resolved.seed_urls)
                if resolved.crawl_strategy == "seed_urls"
                else _effective_max_pages
            )
            ...
            def on_page(
                written: int,
                current_url: str,
                _name: str = source_name,
                _max: int = _scrape_total,
                _task: TaskID = crawl_task,
            ) -> None:
                short_url = (
            ...
            _effective_flat_files = (
                flat_files if flat_files is not None else resolved.flat_files
            )
            result = run_scrape(
                url=resolved.url,
...
def cmd_export(
    ...
    topic: Annotated[
        str | None, typer.Option("--topic", help="Filter by topic prefix.")
    ] = None,
) -> None:
