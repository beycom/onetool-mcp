...
@pytest.mark.unit
@pytest.mark.core
def test_include_tag_loads_external_file() -> None:
    """!include tag loads content from external file."""
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Create external diagram config
        diagram_config = config_dir / "diagram.yaml"
        diagram_config.write_text(
            yaml.dump(
                {
                    "backend": {
                        "prefer": "self_hosted",
        ...
        # Verify included config was loaded
        assert config.tools.diagram.backend.prefer == "self_hosted"
        assert config.tools.diagram.backend.timeout == 60.0
        assert config.tools.diagram.output.dir == "./custom/diagrams"


@pytest.mark.unit
@pytest.mark.core
def test_include_tag_missing_file_for_optional_field() -> None:
    """!include with missing file for optional field logs warning but continues."""
    from ot.config.loader import load_config
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Create main config with !include for prompts (optional/string field)
        # This tests that missing !include returns None and is handled gracefully
        # for fields that can accept None
        config_path = config_dir / "ot-serve.yaml"
        config_path.write_text("version: 1\nprompts: !include nonexistent.yaml\n")

        config = load_config(config_path)

        # prompts should be None (the included file didn't exist)
        assert config.prompts is None


@pytest.mark.unit
@pytest.mark.core
def test_include_tag_missing_file_for_required_field_fails() -> None:
    """!include with missing file for required object field fails validation."""
    from ot.config.loader import load_config
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Create main config with !include to non-existent file for diagram
        # (which expects a dict/DiagramConfig, not None)
        config_path = config_dir / "ot-serve.yaml"
        config_path.write_text(
            "version: 1\ntools:\n  diagram: !include nonexistent.yaml\n"
        )

        # Should fail validation because diagram can't be None
...
@pytest.mark.unit
@pytest.mark.core
def test_include_tag_nested() -> None:
    """!include supports nested includes."""
    from ot.config.loader import load_config
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        backend_config = config_dir / "backend.yaml"
        ...
        # Create diagram config that includes backend
        diagram_config = config_dir / "diagram.yaml"
        diagram_config.write_text(
        ...
        assert config.tools.diagram.backend.prefer == "auto"
        assert config.tools.diagram.backend.timeout == 45.0
