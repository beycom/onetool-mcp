...
class TestKnowledgeDBSchema:
    ...
    def test_fts5_trigger_insert_syncs(self):
        ...
        rows = conn.execute(
            "SELECT topic FROM chunks_fts WHERE chunks_fts MATCH 'lazy'"
        ).fetchall()
        assert len(rows) == 1
    ...
    def test_fts5_trigger_update_syncs(self):
        ...
        _insert_chunk(conn, "c1", "python/generators", "yield expressions are lazy")
        conn.execute(
            "UPDATE chunks SET content = 'coroutines and async' WHERE id = 'c1'"
        )
        conn.commit()
        rows = conn.execute(
            "SELECT topic FROM chunks_fts WHERE chunks_fts MATCH 'coroutines'"
        ).fetchall()
        assert len(rows) == 1
    ...
    def test_topic_allows_duplicates_for_indexed_chunks(self):
        ...
        conn.commit()
        rows = conn.execute(
            "SELECT id FROM chunks WHERE topic = 'same/topic'"
        ).fetchall()
        assert len(rows) == 2
    ...
    def test_embedding_route_rejects_non_positive_dimensions(self):
...
class TestKnowledgeChunker:
    ...
    def test_sidecar_overrides_frontmatter(self, tmp_path: Path):
        ...
        sidecar.write_text(
            "url: https://sidecar-url.example.test/\nsource: crawl\n", encoding="utf-8"
        )
    ...
    def test_sidecar_url_in_meta_but_topic_from_path(self, tmp_path: Path):
        ...
        assert (
            chunks[0].meta.get("url")
            == "https://docs.example.test/app/v1/guide/en-us/commands/move"
        )
...
class TestKnowledgeIndexer:
    ...
    def test_link_graph_inserts_edges(self, tmp_path: Path):
        ...
        _insert_chunk(
            ...
            src_id,
            "a/page",
            "See [move](https://docs.example.test/b/page)",
        ...
        edge = conn.execute(
            "SELECT src_id, dst_id FROM edges WHERE edge_type='link'"
        ).fetchone()
...
class TestKnowledgeSearch:
    ...
    def _conn_with_data(self) -> sqlite3.Connection:
        ...
        _insert_chunk(
            conn, "c3", "python/list", "list comprehension filter map", meta="{}"
        )
        conn.commit()
        return conn
    ...
    def test_rrf_merge_combines_results(self):
        ...
        list_a = [
            {"id": "x", "topic": "t/x", "score": 0.9},
            {"id": "y", "topic": "t/y", "score": 0.5},
        ]
        list_b = [
            {"id": "y", "topic": "t/y", "score": 0.8},
            {"id": "z", "topic": "t/z", "score": 0.3},
        ]
    ...
    def test_metadata_filter_source(self):
        ...
        results = [
            {
                "id": "a",
                "meta_dict": {"source": "docs.test.invalid"},
                "tags_list": [],
                "created_at": "2024-01-01",
            },
            {
                "id": "b",
                "meta_dict": {"source": "other.com"},
                "tags_list": [],
                "created_at": "2024-01-01",
            },
        ]
...
class TestFillImgAlt:
    ...
    def test_empty_alt_replaced_with_stem(self):
        ...
        result = _fill_img_alt(
            "![](https://example.test/icons/propertiesbutton_viewport.png)"
        )
        assert (
            result
            == "![propertiesbutton viewport](https://example.test/icons/propertiesbutton_viewport.png)"
        )
    ...
    def test_hyphens_converted_to_spaces(self):
        ...
        assert (
            result == "![zoom in button](https://example.test/img/zoom-in-button.png)"
        )
    ...
    def test_chunk_file_fills_alt(self, tmp_path: Path):
        ...
        md = tmp_path / "page.md"
        md.write_text(
            "![](https://example.test/icons/move_tool.png) Use the move tool.\n"
        )
...
class TestKnowledgeChunkerStubFilter:
    ...
    def _make_long_file(
        self, lines: list[str], tmp_path: Path, name: str = "doc.md"
    ) -> Path:
        md = tmp_path / name
        md.write_text("\n".join(lines) + "\n", encoding="utf-8")
        return md
...
class TestKnowledgeConfig:
    ...
    def test_dbs_returns_names_when_configured(self):
        ...
        with patch("otutil.tools._knowledge.listing._get_config") as mock_cfg:
            cfg = MagicMock()
            kb_proj = KBProjectConfig(
                db=DBConfig(path="mem/docs.db", description="Project docs")
            )
            cfg.kb = {"docs": kb_proj}
            mock_cfg.return_value = cfg
            result = dbs()
...
class TestKnowledgeCRUD:
    ...
    def test_write_invalid_category_returns_error(self):
        ...
        with _patch_crud_conn(conn):
            result = crud.write(
                topic="x/y", content="content", db="test", category="invalid_cat"
            )
        assert "Error" in result
...
class TestKnowledgeCRUDMultiChunk:
    ...
    def test_read_topic_returns_one_chunk(self):
        ...
        conn = self._patch_db()
        self._insert(
            conn, "c1", "cmd/move", "Page content", source_path="path/move", anchor=""
        )
        self._insert(
            conn,
            "c2",
    ...
    def test_read_by_source_path_returns_all_chunks(self):
        ...
        self._insert(
            conn, "c3", "cmd/other", "Other page", source_path="path/other", anchor=""
        )
        with _patch_crud_conn(conn):
    ...
    def test_delete_by_source_path(self):
        ...
        rows = conn.execute(
            "SELECT id FROM chunks WHERE source_path = 'path/move'"
        ).fetchall()
        assert len(rows) == 0
    ...
    def test_update_targets_specific_anchor(self):
        ...
        conn = self._patch_db()
        self._insert(
            conn, "c1", "cmd/move", "Old intro", source_path="path/move", anchor="intro"
        )
        self._insert(
            conn,
            "c2",
    ...
    def test_update_by_id(self):
        ...
        conn = self._patch_db()
        self._insert(
            conn, "c1", "cmd/move", "Old content", source_path="path/move", anchor=""
        )
        self._insert(
            conn,
            "c2",
    ...
    def test_delete_by_id(self):
        ...
        assert (
            conn.execute("SELECT id FROM chunks WHERE id = 'c2'").fetchone() is not None
        )
...
class TestKnowledgeListing:
    ...
    def test_stats_includes_links_enrichments_and_top_accessed(self, tmp_path: Path):
        ...
        conn.execute(
            "INSERT INTO edges (id, src_id, dst_id, edge_type) VALUES ('e1','c2','c1','link')"
        )
        conn.execute(
            "INSERT INTO edges (id, src_id, dst_id, edge_type) VALUES ('e2','c1','c2','link')"
        )
        conn.commit()
...
class TestKnowledgeRetrieval:
    ...
    def _conn_with_graph(self) -> sqlite3.Connection:
        ...
        conn.execute(
            "INSERT INTO edges (id, src_id, dst_id, edge_type) VALUES ('e1','page-a','page-b','link')"
        )
        conn.execute(
            "INSERT INTO edges (id, src_id, dst_id, edge_type) VALUES ('e2','page-b','page-c','link')"
        )
        conn.commit()
        return conn
    ...
    def test_related_outbound_depth1(self):
        ...
        with patch(
            "otutil.tools._knowledge.retrieval.get_connection", return_value=conn
        ):
...
class TestKnowledgeBatchEmbedding:
    ...
    def test_single_batch_returns_all_vectors(self):
        ...
        with patch(
            ...
            result = generate_embeddings_batch(
                ["hello", "world", "foo"], batch_size=500
            )
    ...
    def test_store_embeddings_batch_is_best_effort(self):
        ...
        with patch(
            "otutil.tools._knowledge.db._check_vec_available", return_value=False
        ):
    ...
    def test_store_embeddings_batch_partial_failure(self):
        ...
        # All 4 chunks stored
        stored = {
            row[0] for row in conn.execute("SELECT chunk_id FROM chunks_vec").fetchall()
        }
    ...
    def test_fallback_aborts_on_consecutive_api_failures(self):
        ...
        from otutil.tools._knowledge.indexer import (
            _FALLBACK_ABORT_AFTER,
...
class TestKnowledgeCLI:
    ...
    def test_reindex_calls_reindex(self):
        ...
        with patch(
            "otutil.tools._knowledge.indexing.reindex",
            return_value="Reindexed 10 chunks",
        ) as mock_re:
            from typer.testing import CliRunner
    ...
    def test_stats_prints_stats(self):
        ...
        with patch(
            "otutil.tools._knowledge.listing.stats", return_value="Stats for 'testdb'"
        ) as mock_s:
            from typer.testing import CliRunner
    ...
    def test_info_prints_info(self):
        ...
        with patch(
            "otutil.tools._knowledge.listing.info", return_value="Database: testdb"
        ) as mock_i:
            from typer.testing import CliRunner
    ...
    def test_export_calls_export_db(self, tmp_path: Path):
        ...
        with patch(
            "otutil.tools._knowledge.listing.export_db",
            return_value="Exported 42 chunks",
        ) as mock_ex:
            from typer.testing import CliRunner

            result = CliRunner().invoke(
                kb_app, ["export", "testdb", "--output", str(out)]
            )

        assert result.exit_code == 0, result.output
        mock_ex.assert_called_once_with(
            db="testdb", path=str(out), category=None, topic=None
        )
...
class TestResolveImageUrls:
    ...
    def test_relative_path_resolved(self):
        ...
        result = _resolve_image_urls(
            "![alt](images/fig.png)", "https://docs.example.test/guide/page"
        )
    ...
    def test_root_relative_path_resolved(self):
        ...
        result = _resolve_image_urls(
            "![x](/assets/img.png)", "https://docs.example.test/guide/page"
        )
...
class TestScraperSlug:
    ...
    def test_strip_html_extension(self):
        ...
        assert (
            url_to_slug("https://docs.example.test/guide/intro.html") == "guide/intro"
        )
...
class TestScraperSidecar:
    def test_named_source_sidecar(self, tmp_path: Path):
        ...
        _write_page(
            tmp_path, "https://docs.example.test/intro", "# Hello\n\nWorld.", "mysite"
        )
    ...
    def test_md_file_contains_content(self, tmp_path: Path):
        ...
        _write_page(
            tmp_path, "https://docs.example.test/api", "## API\n\nDocs.", "site"
        )
    ...
    def test_depth_not_written_to_sidecar(self, tmp_path: Path):
        ...
        _write_page(
            tmp_path, "https://docs.example.test/a/b/c.html", "Content.", "site"
        )
...
class TestResolveSource:
    ...
    def test_bool_false_uses_is_not_none(self):
        ...
        src = ScrapeSourceConfig(
            url="https://docs.example.test/", check_robots_txt=False
        )
        resolved = resolve_source(proj, "s", src)
...
class TestScrapeProjectCLI:
    ...
    def _make_cfg(self, sources: dict[str, Any] | None = None):
        ...
        sources = sources or {
            "src-a": ScrapeSourceConfig(url="https://docs.example.test/a"),
            "src-b": ScrapeSourceConfig(url="https://docs.example.test/b"),
        }
        proj = ScrapeProjectConfig(output_base_dir="/tmp/base", sources=sources)
        return Config(
            kb={
                "myproject": KBProjectConfig(
                    db=DBConfig(path="mem/test.db"), scrape=proj
                )
            }
        )
    ...
    def test_playwright_probe_closes_browser_when_scrape_fails(self):
        ...
        cfg = self._make_cfg(
            sources={"src-a": self._make_cfg().kb["myproject"].scrape.sources["src-a"]}
        )
        ...
        with (
            patch("otutil.tools._knowledge.config._get_config", return_value=cfg),
            patch(
                "otutil.tools._knowledge.scraper.run_scrape",
...
class TestScrapeResume:
    def _make_cfg(self, sources: dict[str, Any]):
        ...
        proj = ScrapeProjectConfig(output_base_dir="/tmp/base", sources=sources)
        return Config(
            kb={"proj": KBProjectConfig(db=DBConfig(path="mem/test.db"), scrape=proj)}
        )
    ...
    def test_mixed_sources_resume_per_source(self, tmp_path: Path):
        ...
        with (
            ...
            patch(
                "otutil.tools._knowledge.scraper.run_scrape",
                side_effect=capture_run_scrape,
            ),
            patch("otutil.tools._knowledge.scraper.write_run_report"),
            patch.dict(sys.modules, _make_fake_playwright_modules()),
        ):
...
class TestResolveSourceNewFields:
    ...
    def test_source_wait_for_overrides_project(self):
        ...
        src = ScrapeSourceConfig(
            url="https://docs.example.test/", wait_for="css:.topic-body"
        )
        resolved = resolve_source(proj, "s", src)
...
class TestScrapeAsyncBridge:
    async def test_run_scrape_completes_inside_running_loop(self, tmp_path: Path):
        ...
        with patch(
            "otutil.tools._knowledge.scraper._run_scrape_async", side_effect=fake_run
        ):
            result = run_scrape(
    ...
    async def test_run_scrape_propagates_async_exception(self, tmp_path: Path):
        ...
        with (
            patch(
                "otutil.tools._knowledge.scraper._run_scrape_async",
...
def _make_fake_crawl4ai_modules(
    *, deep: bool = False
...
class TestScraperContextManagerCleanup:
    async def test_seed_url_scrape_closes_crawler_when_write_fails(
        self, tmp_path: Path
    ):
        import sys
        ...
        with (
            patch.dict(sys.modules, fake_modules),
            patch(
                "otutil.tools._knowledge.scraper._write_page",
...
class TestRunStartWarnings:
    ...
    def _invoke_scrape(
        self, source_cfg_kwargs: dict, project_cfg_kwargs: dict | None = None
    ) -> Any:
        import sys
        from unittest.mock import MagicMock, patch
        ...
        proj_kwargs = {
            "output_base_dir": "/tmp/base",
            "sources": {"src": ScrapeSourceConfig(**source_cfg_kwargs)},
        }
        ...
        with (
            patch("otutil.tools._knowledge.config._get_config", return_value=cfg),
            patch(
                "otutil.tools._knowledge.scraper.run_scrape", return_value=mock_result
            ),
            patch(
                "otutil.tools._knowledge.scraper.write_run_report",
...
class TestCrawlStrategy:
    ...
    def test_seed_urls_strategy_requires_seed_urls(self):
        ...
        src = ScrapeSourceConfig(
            url="https://docs.example.test/", crawl_strategy="seed_urls"
        )
...
class TestCssSelector:
    def test_default_css_selector_is_empty(self):
        ...
        resolved = resolve_source(
            proj, "s", ScrapeSourceConfig(url="https://docs.example.test/")
        )
    ...
    def test_css_selector_preserved_on_source(self):
        ...
        src = ScrapeSourceConfig(
            url="https://docs.example.test/", css_selector="#mc-main-content"
        )
        resolved = resolve_source(proj, "s", src)
...
class TestScrapeProgressDenominator:
    ...
    def _compute_total(
        self, crawl_strategy: str, seed_urls: list, max_pages: int
    ) -> int:
...
class TestScraperWritePageCategoryTags:
    ...
    def test_category_written_when_set(self, tmp_path: Path):
        ...
        _write_page(
            tmp_path, "https://docs.example.test/p", "Content.", "src", category="rule"
        )
...
class TestCanonicalize:
    ...
    def test_source_dir_strips_prefix(self):
        ...
        assert (
            canonicalize("app/v1/help/commands/move.md", source_dir="app/v1/help")
            == "commands/move"
        )
...
class TestKBProjectConfig:
    ...
    def test_kb_project_config_round_trip(self):
        from otutil.tools._knowledge.config import Config, DBConfig, KBProjectConfig
    ...
    def test_unknown_scrape_key_raises_schema_error(self):
        ...
        with pytest.raises(ValidationError, match="Extra inputs are not permitted"):
            Config(
                scrape={
                    "docs": ScrapeProjectConfig(
                        output_base_dir="/tmp",
                        sources={
                            "s": ScrapeSourceConfig(url="https://docs.example.test/")
                        },
                    )
                }
            )
...
class TestScraperFlatFiles:
    ...
    def test_flat_files_false_creates_subdirectory(self, tmp_path: Path):
        ...
        _write_page(
            tmp_path,
            "https://docs.example.test/guide/intro",
            "Content.",
            "src",
            flat_files=False,
        )
    ...
    def test_flat_files_true_deep_path(self, tmp_path: Path):
        ...
        _write_page(
            tmp_path,
            "https://docs.example.test/a/b/c/page",
            "Body.",
            "src",
            flat_files=True,
        )
...
class TestSearchEmbeddingsGuard:
    ...
    def test_embeddings_disabled_hybrid_degrades_to_keyword_search(self):
        ...
        with (
            ...
            patch.object(retrieval, "search_hybrid") as mock_search_hybrid,
        ):
...
class TestRetrievalUntrustedContextBoundary:
    ...
    def test_synthesise_includes_untrusted_system_message(self):
        ...
        generated = MagicMock(return_value=MagicMock(content="answer"))
        with (
            patch.object(retrieval, "resolve_generation", return_value=MagicMock()),
            patch.object(retrieval, "generate", generated),
        ):
            retrieval._synthesise("q", "some context")

        request = generated.call_args.kwargs["request"]
    ...
    def test_llm_rerank_includes_untrusted_system_message(self):
        ...
        generated = MagicMock(return_value=MagicMock(content="5"))
        with (
            patch.object(retrieval, "resolve_generation", return_value=MagicMock()),
            patch.object(retrieval, "generate", generated),
        ):
    ...
    def test_ask_expand_false_excludes_neighbours(self):
        ...
        with (
            ...
            result = retrieval.ask(
                query="q", db="test", k=1, rerank=False, expand=False
            )
...
class TestSearchDegradation:
    ...
    def test_search_vec_missing_table_raises(self):
        ...
        with (
            ...
            patch(
                "otutil.tools._knowledge.search.generate_embedding",
                return_value=[0.0] * 4,
            ),
    ...
    def test_ask_degrades_to_fts_with_warning(self):
        ...
        with (
            patch.object(retrieval, "get_connection", return_value=conn),
            patch.object(
                retrieval, "search_hybrid", side_effect=RuntimeError("vec broken")
            ),
            ...
            def execute(self, sql: str, params: object = None):
                if sql.strip().startswith("INSERT INTO chunks"):
                    raise sqlite3.IntegrityError(
                        "UNIQUE constraint failed: chunks.topic"
                    )
...
@pytest.mark.unit
@pytest.mark.tools
class TestIndependentEmbeddingClient:
    """Knowledge embeddings use only the independent embedding route."""

    def test_embedding_client_uses_route_timeout(self):
        from otutil.tools._knowledge import embedding as emb_mod

        route = EmbeddingsConfig(
            backend="openai_compatible",
            model="text-embedding-3-small",
            base_url="https://api.openai.com/v1",
            secret_name="EMBEDDINGS_API_KEY",
            dimensions=1536,
            timeout=17,
        )
        ...
        with (
            patch(
                "otutil.tools._knowledge.embedding.get_secret", return_value="sk-test"
            ),
            patch(
                "otutil.tools._knowledge.embedding.get_embeddings_config",
                return_value=route,
            ),
        ):
            client = emb_mod._get_embedding_client()

        assert client._timeout == 17
        assert client.model == "text-embedding-3-small"
        emb_mod._client = None
        emb_mod._client_key = None

    def test_missing_embedding_route_fails_before_secret_lookup(self):
        from otutil.tools._knowledge import embedding as emb_mod
        ...
        with (
            ...
            patch("otutil.tools._knowledge.embedding.get_secret") as secret,
            pytest.raises(ValueError, match="Top-level embeddings"),
        ):
...
class TestScrapeResumeState:
    ...
    async def test_deep_crawl_resume_passes_state_file_when_absent(
        self, tmp_path: Path
    ):
        import sys
    ...
    async def test_seed_urls_resume_skips_done_and_updates_state(self, tmp_path: Path):
        ...
        (tmp_path / ".state.json").write_text(
            _json.dumps({"done_urls": ["https://docs.example.test/a"]}),
            encoding="utf-8",
        )
        with patch.dict(sys.modules, fake_modules):
            result = await _run_scrape_async(
                **self._kwargs(
                    tmp_path,
                    crawl_strategy="seed_urls",
                    seed_urls=[
...
def _mock_generation(reply: str = "A concise summary.") -> MagicMock:
    return MagicMock(return_value=MagicMock(content=reply))
...
def _patch_enrichment(
    ...
    with (
        ...
        patch(
            "otutil.tools._knowledge.enrichment.resolve_generation",
            return_value=MagicMock(),
        ),
        patch("otutil.tools._knowledge.enrichment.generate", generation),
    ):
        yield
...
class TestEnrichDB:
    ...
    def test_backfill_selects_only_missing_summaries(self):
        ...
        with _patch_enrichment(conn, _mock_enrich_config(), generation):
        ...
        assert (
            conn.execute("SELECT summary FROM chunks WHERE id='c3'").fetchone()[0]
            == "already done"
        )
    ...
    def test_force_reprocesses_all(self):
        ...
        generation = _mock_generation("fresh summary")
        with _patch_enrichment(conn, _mock_enrich_config(), generation):
            result = enrich_db(db_name="test", force=True)

        assert result.enriched == 1
        assert (
            conn.execute("SELECT summary FROM chunks WHERE id='c1'").fetchone()[0]
            == "fresh summary"
        )
    ...
    def test_short_chunk_skipped_with_empty_summary(self):
        ...
        with _patch_enrichment(
            conn,
            _mock_enrich_config(enrich_min_chars=400),
            generation,
        ):
            result = enrich_db(db_name="test")
    ...
    def test_consecutive_failures_abort_run(self):
        ...
        generation = MagicMock(side_effect=RuntimeError("boom"))
        with _patch_enrichment(conn, _mock_enrich_config(), generation):
            result = enrich_db(db_name="test")
    ...
    def test_per_batch_commit_survives_crash(self, tmp_path: Path):
        ...
        good = generation.return_value
        generation.side_effect = [good, good, KeyboardInterrupt()]
        with (
            _patch_enrichment(
                conn,
                _mock_enrich_config(enrich_batch_size=2),
                generation,
            ),
    ...
    def test_updated_at_unchanged_by_enrichment(self):
        ...
        _insert_chunk(conn, "c1", "a/one", "Content one")
        conn.execute(
            "UPDATE chunks SET updated_at = '2020-01-01 00:00:00' WHERE id='c1'"
        )
        conn.commit()

        with _patch_enrichment(conn, _mock_enrich_config(), _mock_generation()):
            enrich_db(db_name="test")

        assert (
            conn.execute("SELECT updated_at FROM chunks WHERE id='c1'").fetchone()[0]
            == "2020-01-01 00:00:00"
        )
    ...
    def test_generation_configuration_failure_is_reported(self):
        from otutil.tools._knowledge.enrichment import enrich_db
    ...
    def test_transient_generation_failure_retries_same_route(self):
        from otutil.tools._knowledge.enrichment import enrich_db
        ...
        generation = MagicMock(
            side_effect=[
                GenerationError("temporary failure", status_code=503),
                MagicMock(content="Recovered summary."),
            ]
        )
        with (
            _patch_enrichment(conn, _mock_enrich_config(), generation),
            patch("otutil.tools._knowledge.enrichment.time.sleep") as sleep,
        ):
            result = enrich_db(db_name="test")
    ...
    def test_reindex_changed_content_clears_summary(self, tmp_path: Path):
        ...
        try:
            with (
                ...
                summaries = [
                    r[0] for r in conn.execute("SELECT summary FROM chunks").fetchall()
                ]
...
class TestSummaryInSearch:
    ...
    def test_fts_matches_summary_only_term(self):
        ...
        conn.execute(
            "UPDATE chunks SET summary = 'Covers zebrafish handling' WHERE id='c1'"
        )
        conn.commit()
    ...
    def test_search_shows_summary_or_extract(self):
        ...
        _insert_chunk(
            conn, "c1", "a/one", "summarised chunk searchterm " + long_content
        )
    ...
    def test_ask_synthesis_uses_raw_content(self):
        ...
        with (
            patch(
                "otutil.tools._knowledge.retrieval.get_connection",
                return_value=MagicMock(),
            ),
            patch(
                "otutil.tools._knowledge.retrieval.search_hybrid", return_value=results
            ),
            patch(
                "otutil.tools._knowledge.retrieval._synthesise",
                side_effect=fake_synthesise,
            ),
        ):
...
class TestEnrichCLIWiring:
    ...
    def test_index_enrich_passes_only_run_chunk_ids(self, tmp_path: Path):
        ...
        assert mock_enrich.call_args.kwargs["model"] == "sol"
        assert mock_enrich.call_args.kwargs["effort"] == "high"
    ...
    def test_default_index_makes_no_llm_calls(self, tmp_path: Path):
        ...
        with (
            patch(
                "otutil.tools._knowledge.indexer.index_directory",
                return_value=IndexResult(indexed=1),
            ),
            patch("otutil.tools._knowledge.enrichment.enrich_db") as mock_enrich,
        ):
