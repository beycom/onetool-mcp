...
- ``fake_render_engine``: monkeypatches
  ``otdev.tools._arch.generate._execute_render_engine`` with a fake that
  writes a placeholder SVG instead of shelling out to ``d2``.
...
def fake_render_engine(monkeypatch: pytest.MonkeyPatch) -> Callable[..., None]:
    """Replace ``otdev.tools._arch.generate._execute_render_engine`` with a fake render.
